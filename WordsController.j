@implementation WordsController : CPObject
{
...
- (id)init
{
	if(self = [super init])
	{
		...
		objsToDisplay = [];
		//[self getWord:@""];
		//console.log(objsToDisplay);
        

	}
	return self;
}
- (void)getWord:(CPString)aGroup
{
	if(aGroup == allName)
		aGroup = "";
...
- (void)tableView:(CPTableView)tableView setObjectValue: (CPControl)anObject forTableColumn:(CPTableColumn)aTableColumn row:(int)rowIndex
{
    [objsToDisplay[rowIndex] setObject:anObject forKey:[aTableColumn identifier]];
    //[objsToDisplay[2] setObject:anObject forKey:[aTableColumn identifier]];
